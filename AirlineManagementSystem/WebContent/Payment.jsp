 ...
 <div class="container">
  <form class="form-signin" method="post" action=PaymentServlet >
  		<% if(request!=null && request.getAttribute("total_cost")!=null) { %>
   		<span  style="font-size: 15px; " class="label label-info">Your total amount due: <%=request.getAttribute("total_cost").toString() %></span>
   		<% } %>
   		...
   		<% } %>
   		<br><br>
        <span  style="font-size: 25px; margin-left:50px;" class="label label-primary">Payment</span>
        <br>
        
		<br><br><br>
		<input  id="ccno" name="ccno" type="text" class="form-control" placeholder="Credit Card No" required autofocus pattern="[0-9]{16}" />
		
         <input type="text" name="ccname" class="form-control" placeholder="Name" required  pattern="[A-Z\sa-z]{2,30}" />
        <br>
         <input type="text" name="exp_date"  class="form-control" placeholder="Expiry Date" required="required" pattern="[0-9]{2}/[0-9]{2}"/>
       <br>
       	<input type="text" name="cvv" id="cvvv" class="form-control" placeholder="CVV" required="required" pattern="[0-9]{3}"/>
       <br>
       <% if(request!=null && request.getAttribute("flightNo")!=null) { %>
       	<input type="hidden" name="flightNo" id="flightNo" value="<%=request.getAttribute("flightNo")%>"/>
       <%} %>
       <% if(request!=null && request.getAttribute("flightDate")!=null) { %>
       	<input type="hidden" name="flightDate" id="flightDate" value="<%=request.getAttribute("flightDate")%>"/>
       <%} %>
       <% if(request!=null && request.getAttribute("noofseats")!=null) { %>
       	<input type="hidden" name="noofseats" id="noofseats" value="<%=request.getAttribute("noofseats")%>"/>
       <%} %>
       <% if(request!=null && request.getAttribute("total_cost")!=null) { %>
       	<input type="hidden" name="total_cost" id="total_cost" value="<%=request.getAttribute("total_cost")%>"/>
       <%} %>
