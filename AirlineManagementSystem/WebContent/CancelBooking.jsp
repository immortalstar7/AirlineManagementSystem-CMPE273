...
function validate(){
	...
	if(noOfSeats==0) alert("Invalid number of seats");
		...
		alert("You can cancel upto 5 seats only.");
}
</script>
</head>
<body>
...
<!-- Navigation Ends -->
 <div class="container">
  <form class="form-signin" method="post" action=cancelbooking  onsubmit="validate()">
  		<% if(request!=null && request.getAttribute("status")!=null) { %>
   		...
   		<% } %>
   		<br><br>
        <span  style="font-size: 25px; margin-left:50px;" class="label label-primary">Cancel Booking</span>
        <br><br> <br>
        <input type="text" name="flightNumber" class="form-control" placeholder="Flight Number" pattern="[A-Za-z]{3}[0-9]{5}" required="required">
        <input id="flyingDate" name="flyingDate" class="form-control" type="text" placeholder="Flight Date" class="input-large" required>
        <input type="text" name="numSeats" id="numSeats" class="form-control"  placeholder="No Of Seats" required="required" pattern="[0-9]{1,2}">
         
       <br>
       
        <button class="btn btn-lg btn-primary btn-block" type="submit">Cancel Booking</button>
